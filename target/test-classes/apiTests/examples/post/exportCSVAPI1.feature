@sanity
Feature: Check Campaign Check Export CSV APi Response with One year Range
  Background:
    * url 'https://demo-node.accushield.com'

  @sampleAPIs
  Scenario: Check Campaign Check Export CSV APi Response with One year Range
    #    * header Authorization = "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..gJ5CliDDIXvQOZJpGigjR5t1UN_Bx7T59u-vOebduOY"
    * header Content-Type = 'application/json'
    * header Authorization = token
    Given path 'CampaignResultsPage/exportCampaignReportListCSV'
    And param lang = "fr"
    And request '{"community_id":1243,"start_date":"2022-01-03","end_date":"2023-04-02","communityTimeZone":"America/New_York"}'
    When method post
    Then status 200
    * print response
    And match response.statusCode == 200